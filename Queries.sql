...
HAVING COUNT(*) FILTER (WHERE EXTRACT(MONTH FROM day) = EXTRACT(MONTH FROM CURRENT_DATE)) > 1
...
INNER JOIN number_booked_students ON schedule.id = number_booked_students.id 
WHERE EXTRACT(WEEK FROM schedule.day) = EXTRACT(WEEK FROM CURRENT_DATE + 7) 
ORDER BY genre.genre ASC, day ASC;
